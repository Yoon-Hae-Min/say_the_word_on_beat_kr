...
이 커맨드는 **architect** 에이전트를 호출하여 코드를 작성하기 전에 포괄적인 구현 계획을 만듭니다.
...
**핵심**: architect 에이전트는 "yes"나 "proceed" 같은 긍정적 응답으로 명시적으로 계획을 확인하기 전까지 코드를 **절대 작성하지 않습니다.**
...
이 커맨드는 다음 위치의 `architect` 에이전트를 호출합니다:
